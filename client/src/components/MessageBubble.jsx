        ...
        <div className={`${isUser ? 'flex justify-end' : 'flex-1'}`}>
          {isTyping ? (
            ...
            <div className={`${
              ...
              {/* Arquivos anexados - priorizar metadata.attachments sobre files */}
              {(message.metadata?.attachments && message.metadata.attachments.length > 0) ? (
                <div className="mb-3 flex flex-wrap gap-2">
                  {message.metadata.attachments.map((attachment, index) => (
                    <div key={`attachment-${index}`} className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 px-3 py-2 rounded-lg">
                      <File size={16} className="text-gray-600 dark:text-gray-400" />
                      <span className="text-sm text-gray-700 dark:text-gray-300">{attachment.originalname}</span>
